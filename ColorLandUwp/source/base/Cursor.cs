           ...
           /* for (int i = 0; i < tracers.Length; i++)
            {
                if (tracers[i].alive)
                {
                    spriteBatch.Draw(getCurrentSprite().getCurrentTexture2D(), new Rect((int)tracers[i].pos.X, (int)tracers[i].pos.Y, 32, 32), Color.White * tracers[i].alpha);
                }
            }
            */
            ...
            else
            {
                ...
                {
                    base.draw(spriteBatch, mRotation += 0.8f, new Rect((int)mX + 40, (int)mY + 40, 80, 80), new Vector2(80, 80), Color.White * mAlphaClick); //0.4
                }

            }
            //spriteBatch.Draw(getCurrentSprite().getCurrentTexture2D(), new Vector2(mX, mY), new Rect(0, 0, getCurrentSprite().getWidth(), getCurrentSprite().getHeight()), Color.White, mRotation+=0.3f, new Vector2(300, 300), 1, SpriteEffects.None, 0);
