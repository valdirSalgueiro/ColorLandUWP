...
namespace ColorLand
{
    class EnemySimpleShooting : BaseEnemy
    {
        ...
        public override void draw(SpriteBatch spriteBatch)
        {
            ...
            {
                if (bullet[i].active)
                    spriteBatch.Draw(bullet[i].texture, new Rect((int)bullet[i].pos.X + bullet[i].texture.Width / 2, (int)bullet[i].pos.Y + bullet[i].texture.Height / 2, bullet[i].texture.Width, bullet[i].texture.Height), Color.White);
            }
        }
